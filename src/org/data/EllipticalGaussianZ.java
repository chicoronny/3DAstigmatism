...
public class EllipticalGaussianZ {
	...
	public double[] getInitialGuess(ImageProcessor ip, Roi roi) {
	    ...
	    initialGuess[INDEX_Z0] = d*d*(centroid[INDEX_SY]*centroid[INDEX_SY]-centroid[INDEX_SX]*centroid[INDEX_SX])/(4*w0*w0*c)+param[INDEX_Mp];
	...
	public double Ex(double x, double[] variables){
		double tsx = Math.sqrt(2)*Sx(variables[INDEX_Z0]);
	...
	public double Ey(double y, double[] variables){
		double tsy = Math.sqrt(2)*Sy(variables[INDEX_Z0]);
	...
	public double dSx(double z){
		...
		double d = param[INDEX_D];
		double b = (z-param[INDEX_C]-param[INDEX_Mp])/d;
