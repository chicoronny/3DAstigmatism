...
class EllipticalGaussianZ implements OptimizationData {
	...
	private double Ex(double x, double[] variables) {
		final double tsx = sqrt2 * Sx(variables[INDEX_Z0]);
		final double xm = x - variables[INDEX_X0] - 0.5;
		final double xp = x - variables[INDEX_X0] + 0.5;
		return 0.5 * erf(xp / tsx) - 0.5 * erf(xm / tsx);
	}

	private double Ey(double y, double[] variables) {
		final double tsy = sqrt2 * Sy(variables[INDEX_Z0]);
		final double ym = y - variables[INDEX_Y0] - 0.5;
		final double yp = y - variables[INDEX_Y0] + 0.5;
