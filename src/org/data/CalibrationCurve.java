...
public class CalibrationCurve {
	...
	public double[] getInitialGuess() {
		...
		initialGuess[INDEX_D] = Math.abs(zgrid[minIndexx]) + Math.abs(zgrid[minIndexy]); // / is that a good guess??
		...
		return new MultivariateMatrixFunction() {
			public double[][] value(double[] params) {
				...
				for (int i = 0; i < length; ++i) {
					...
					jacobian[i][INDEX_C] = -0.5 * params[INDEX_WX]
							* (2 * b + 3 * params[INDEX_AX] * b * b + 4 * params[INDEX_BX] * b * b * b)
							/ (params[INDEX_D] * denom);
				...
				for (int i = length; i < 2 * length; ++i) {
					...
					jacobian[i][INDEX_C] = 0.5 * params[INDEX_WY]
							* (2 * b + 3 * params[INDEX_AY] * b * b + 4 * params[INDEX_BY] * b * b * b)
							/ (params[INDEX_D] * denom);
					jacobian[i][INDEX_D] = -b * jacobian[i][INDEX_C];
				}
