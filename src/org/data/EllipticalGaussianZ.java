...
public class EllipticalGaussianZ {
	...
	public EllipticalGaussianZ(int[] xgrid, int[] ygrid, Calibration cal){
		this.xgrid = xgrid;
		this.ygrid = ygrid;
		this.cal = cal;
	}
	...
	public double dEsx(double x, double[] variables) {
		...
		return 0.5
				* tsx
				* ((x - variables[INDEX_X0] - 0.5) * dErf(tsx * (x - variables[INDEX_X0] - 0.5)) - (x
						- variables[INDEX_X0] + 0.5)
						* dErf(tsx * (x - variables[INDEX_X0] + 0.5))) / Sx(variables[INDEX_Z0]);
	}
	...
	public double dEsy(double y, double[] variables) {
		...
		return 0.5
				* tsy
				* ((y - variables[INDEX_Y0] - 0.5) * dErf(tsy * (y - variables[INDEX_Y0] - 0.5)) - (y
						- variables[INDEX_Y0] + 0.5)
						* dErf(tsy * (y - variables[INDEX_Y0] + 0.5))) / Sy(variables[INDEX_Z0]);
	}

	public double Sx(double z) {
		return cal.getValueWx(z);
	}

	public double Sy(double z) {
		return cal.getValueWy(z);
	}

	public double dSx(double z) {
		double value;
		double[] param = cal.getparam();
		double A = param[INDEX_AX];
		double B = param[INDEX_BX];
		double d = param[INDEX_D];
		double b = (z - param[INDEX_C]) / d;
		value = 0.5 * param[INDEX_WX] * param[INDEX_WX] * (2 * b / d + 3 * A * b * b / d + 4 * B * b * b * b / d)
				/ cal.getValueWx(z);
		return value;
	}
	...
	public double dSy(double z) {
		...
		double d = param[INDEX_D];
		double b = (z - param[INDEX_C]) / d;
		value = 0.5 * param[INDEX_WY] * param[INDEX_WY] * (2 * b / d + 3 * A * b * b / d + 4 * B * b * b * b / d)
				/ cal.getValueWy(z);
		return value;
	}
	...
	public double[] getInitialGuess(ImageProcessor ip, Roi roi) {
		...
		double d = param[INDEX_D];
	    initialGuess[INDEX_Z0] = d*d*(centroid[INDEX_SY]*centroid[INDEX_SY]-centroid[INDEX_SX]*centroid[INDEX_SX])/(4*w0*w0*c);
